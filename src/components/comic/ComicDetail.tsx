...
const ComicDetails = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`http://localhost:3000/comics/${id}`).then((response) => response.json()),
      fetch(`http://localhost:3000/comics/${id}/genres`).then((response) => response.json())
    ])
  ...
  const nextImage = () => {
    setCurrentImageIndex((prevIndex) => (prevIndex + 1) % comics[0].previewChapter.length);
  };

  const prevImage = () => {
    setCurrentImageIndex((prevIndex) => (prevIndex - 1 + comics[0].previewChapter.length) % comics[0].previewChapter.length);
  };
      ...
      <Grid container spacing={2.5} className='frame'>
        ...
        <Grid size={{ xs: 7 }} className='detail-frame'>
          <div className='detail1' style={{ marginBottom: "20px" }}>
            ...
            <Typography className='title'>{comics.title || 'Tên truyện'}</Typography>
            <div className='author'>
              <Typography className='title1'>
                Bộ: <span className="blue-text">{comics.series || 'Bộ truyện'}</span>
              </Typography>
              <Typography className='title1'>
                Tác giả: <span className="author-name">{comics.author || 'Tác giả'}</span>
              </Typography>
            </div>
          ...
          <div className='detail2'>
            ...
            <div className='authorinfo'>
              ...
              <Typography className='inforight'>{comics.author || 'N/A'}</Typography>
              <div className="divider"></div>
            </div>
            <div className='authorinfo'>
              <Typography className='infoleft'>Ngôn ngữ:</Typography>
              <Typography className='inforight'>{comics.language || 'Chưa có'}</Typography>
              <div className="divider"></div>
            </div>
            <div className='authorinfo'>
              <Typography className='infoleft'>Thể loại:</Typography>
              <Typography className='infogenre'>
                {getGenreNames(comics.genreIds)}
              </Typography>
              <div className="divider"></div>
            </div>
            <Typography className='note'>
              Giá sản phẩm trên ComZone đã bao gồm thuế theo luật hiện hành. Bên cạnh đó, tuỳ vào loại sản phẩm, hình thức và địa chỉ giao hàng mà có thể phát sinh thêm chi phí khác như: Phụ phí đóng gói, phí vận chuyển, phụ phí hàng cồng kềnh,...
            </Typography>
          </div>
        </Grid>
