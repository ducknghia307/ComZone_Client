...
interface PaymentForDepositProps {
  auctionId: string;
  isModalOpen: boolean;
  handleOk: () => void;
  handleCancel: () => void;
  amount: number;
  onPayment: (method: string, selectedAmount: number) => void;
}

const PaymentForDeposit: React.FC<PaymentForDepositProps> = ({
  auctionId,
  isModalOpen,
  handleOk,
  handleCancel,
    ...
    try {
      if (selectedWalletMethod === "ZaloPay") {
        ...
        const resZalopay = await privateAxios.post("/zalopay", {
          ...
          redirectPath: `/auctiondetail/${auctionId}`,
        });
      ...
      <div className="w-full">
        ...
        <div className="mb-4">
          ...
          <input
            ...
            value={CurrencySplitter(Number(selectedAmount)) || ""}
            onChange={(e) => {
              setSelectedAmount(Number(e.target.value.replace(/[^0-9]/g, "")));
            }}
          />
        ...
        <div className="w-full flex items-center justify-center mt-8">
          <button
            ...
            disabled={!selectedWalletMethod || selectedAmount <= 0}
          >
...
export default PaymentForDeposit;
