...
export default function PriceAndImages({
  ...
  handleGettingImagesAndPrice,
}: {
  currentStep: number;
  mainInformation: ComicMainInformation;
  setCurrentStep: React.Dispatch<React.SetStateAction<number>>;
  editionEvidenceFields: string[];
  handleGettingImagesAndPrice: (value: PriceAndImagesResponse) => void;
}) {
  const [coverImage, setCoverImage] = useState<File>();
  const [coverImagePlaceholder, setCoverImagePlaceholder] = useState<string>();

  const [previewChapters, setPreviewChapters] = useState<File[]>([]);
  ...
  const handleUploadCoverImage = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > 0) {
      ...
      setCoverImagePlaceholder(URL.createObjectURL(files[0]));
    }
  };
    ...
    if (files.length > 0) {
      Array.from(files).map((file, index) => {
        if (index < maxPreviewChapters) {
          setPreviewChapters((prev) => [...prev, file]);
  ...
  const handleRemovePreviewChapters = (index: number) => {
    setPreviewChapters(previewChapters.filter((_, i) => i !== index));
    setPreviewChaptersPlaceholder(
      previewChaptersPlaceholder.filter((_, i) => i !== index)
    );
  };

  const handleSubmitPriceAndImages = () => {
    if (!coverImage) {
      message.error({
        key: "error",
        content: (
  ...
  if (currentStep === 2 && mainInformation)
    return (
      <div className="flex flex-col items-stretch xl:w-2/3 mx-auto gap-4">
        ...
        <div className="space-y-2">
          ...
          <ul className="list-disc px-4">
            <li className="text-sm font-light">
              ...
              :&emsp;Ảnh được sử dụng chính làm hình minh họa cho{" "}
              {mainInformation.quantity > 1 ? "bộ truyện" : "truyện"} của bạn.
            </li>

            <li className="text-sm font-light">
              <span className="font-semibold text-base">
                Ảnh đính kèm <span className="text-red-600">*</span>
              </span>
              &nbsp;(Tối đa {maxPreviewChapters} ảnh):&emsp;Ảnh chụp{" "}
              {mainInformation.quantity > 1 ? "bộ truyện" : "truyện"} theo nhiều
              hình thức khác nhau (mặt sau, các trang truyện, hộp chứa,...).
            </li>
          </ul>
        ...
        <div className="flex flex-col sm:flex-row items-stretch justify-start gap-4">
          ...
          <div className="grid grid-cols-4 items-center justify-center gap-2">
            ...
            {previewChapters.length < maxPreviewChapters && (
              <button
                onClick={() => {
        ...
        <p className="font-semibold uppercase text-lg">2. Giá bán truyện</p>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-start justify-center gap-4 sm:gap-2">
          <label className="basis-1/3 relative cursor-text">
            <input
              ...
              onChange={(e) => {
                if (e.target.value.length === 0) setPrice(undefined);
                else {
                  const priceValue = e.target.value.replace(/\./g, "");
                  if (/^\d+$/.test(priceValue)) {
                    const numberValue = Number(priceValue);
                    if (numberValue > 99999000) setPrice(99999000);
                    else setPrice(numberValue);
                  }
                }
              }}
        ...
        <div className="flex items-center justify-between gap-2 mt-8">
          ...
          <button
            onClick={handleSubmitPriceAndImages}
            className="grow flex items-center justify-center gap-2 px-8 py-2 bg-green-600 text-white rounded duration-200 hover:bg-green-800"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
          ...
          {isConfirming && (
            <ActionConfirm
              ...
              title="Xác nhận tạo thông tin truyện?"
              confirmCallback={() => {
                handleGettingImagesAndPrice({
                  coverImage,
                  previewChapters,
